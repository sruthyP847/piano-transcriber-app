...
Context-handoff for a fresh Claude Code session, zero prior context. Read this before touching code. Reflects `backend/main.py` and `frontend/app/page.tsx` as read in full and verified line-by-line as of commit `6b2260a` ("perspective-correct keyboard calibration with 2D key-shape modeling (Stage 1.5)"), then updated for commit `6914904` (proximity measurement functions), the consolidation pass in `8d27a37` (§3 item 15), and commit `ba38c48` (calibration wiring, §3 item 16). All `main.py:NNN` line references below were re-verified against the working tree at `ba38c48`. If reading this later, run `git log --oneline -10` and sanity-check the commit history still matches before trusting anything below over the actual code — and treat line numbers as approximate, since they drift with every edit.
...
|---|---|---|
| Backend | FastAPI, single file | `backend/main.py` (1498 lines) |
| Frontend | Next.js / React, single file, client component | `frontend/app/page.tsx` (986 lines) |
...
- Video/CV layer: **partly wired as of the calibration-wiring session.** `build_keyboard_calibration()` now has a real call site in `/api/transcribe` (`main.py:1447`). The three measurement functions — `detect_key_press_at_timestamp()`, `detect_hand_canonical_positions()`, `lateral_distance_to_nearest_landmark_mm()` — still have **zero call sites outside their own definitions** (verified via grep). Apart from the calibration build, the only CV-adjacent code that *runs* in the live pipeline is still a 3-line diagnostic print loop (see §2).
- Frontend calibration UI: **fully built and now actually wired.** `uploadFile()` appends a single `calibration` JSON field to the `FormData` alongside `file`/`time_signature`/`tempo_bpm`, carrying the 4 corners, `leftmost_note`/`rightmost_note`, and `frame_width`/`frame_height`. Corners are sent in **source-video pixel coordinates** (`corner.x * previewFrameWidth`, where that width is the extracted frame's natural `video.videoWidth`), *not* displayed-element coordinates — the preview is scaled and letterboxed, so element-relative values would carry the display scale factor as a systematic offset. The frame dimensions are sent so the backend can reject a size mismatch instead of silently misprojecting. Sent as one JSON field rather than ~12 flat fields because calibration is all-or-nothing: a single field is either present or absent, whereas separate fields admit partially-populated states.
  **⚠️ VERIFICATION ONLY.** The backend validates the payload, builds the calibration, and returns a summary of it — **it does not modify any detected note.** `analyze_audio()`'s signature takes no calibration argument and the endpoint does not pass it one (verified). Applying calibration to the notes (proximity vetoing, confidence penalties) remains a separate, undecided step.
...
### `analyze_audio(audio_path, video_path, time_signature=None, tempo_bpm_override=None)` — `main.py:1113`
...
### `POST /api/transcribe` — `main.py:1387`
...
4. If the optional `calibration` Form field is present: validate it (`parse_calibration_payload`) and build the keyboard calibration. Done *before* the analysis so bad input fails fast. Rejects with a specific **400** for malformed/non-object JSON, a missing/non-numeric/non-finite corner, an unparseable note name, an inverted or equal note range, a frame-size mismatch against the actual video, and degenerate corner geometry (zero-area quad, or any three corners collinear — the real singularity condition for `cv2.getPerspectiveTransform`, which a mere "are the points distinct?" test would miss). All 11 of these paths are covered by tests run against the live endpoint.
5. Call `analyze_audio(...)` with `time_signature`/`tempo_bpm` from `Form(...)` params (both optional). **Calibration is deliberately NOT passed** — `analyze_audio` takes no calibration argument, so note detection is provably unaffected.
6. `generate_notation_pdf(events, pdf_path, time_signature=<parsed tuple>)` — real PDF via abjad/LilyPond.
7. `generate_placeholder_musicxml(musicxml_path)` — **still a static, hardcoded single-measure placeholder**, deliberately disconnected from real notes (§4).
8. Return JSON: upload metadata + `pdf_url`/`musicxml_url` + everything from `analyze_audio`'s dict spread in, **plus a `calibration` key only when calibration was supplied** (absent otherwise, so the no-calibration response keeps exactly its previous shape).

### CV-layer function wiring status

| Function | Purpose | Wired? |
|---|---|---|
| `build_keyboard_calibration(leftmost_note, rightmost_note, front_left_px, front_right_px, back_left_px, back_right_px)` | 4-corner perspective homography → per-key pixel quads + `pixel_to_note(x,y)` / `pixel_to_canonical(x,y)` lookups | **Yes** — called from `/api/transcribe` (`main.py:1447`) when the client sends calibration. Result is summarised into the response only; it does not touch the notes. |
| `detect_key_press_at_timestamp(video_path, timestamp, calibration, window_seconds=0.2, fps=None)` | MediaPipe fingertip z-trajectory dip detection → ranked note candidates | No — 0 call sites outside its own definition. **Effectively superseded**: §3 item 14 abandoned the z signal entirely. Kept for reference, not on any current path. |
| `detect_hand_canonical_positions(video_path, timestamp, calibration, landmark_indices=None)` | Single nearest frame, IMAGE mode → hand landmarks projected into canonical keyboard space | No — 0 call sites outside its own definition |
| `lateral_distance_to_nearest_landmark_mm(calibration, midi, points)` | Lateral (u-axis) mm from a note's key centre to the nearest projected landmark; `None` when the note is outside the calibrated range | No — 0 call sites outside its own definition |
| `extract_frame_at_time` | Single-frame extraction | Yes — the diagnostic loop above, and `detect_hand_canonical_positions` |
...
15. **Consolidation pass — a false "basic-pitch octave error" finding, retracted.** Stage 2.5 reported that basic-pitch had mis-registered the final chord of `chords-notes-mix.mp4`, claiming the real notes were C#5/E5 because the video put fingers 0.9mm from C#5 and 73.8mm from C#4. **That finding is WRONG and is retracted.** The user has since confirmed the ground truth by watching the video: the final chord is a **root-position C#dim7 — C#4, E4, G4, A#4 — exactly as basic-pitch reported.** basic-pitch was correct; the video mapping was wrong.
    ...
    - **This failure mode does not exist in production.** The calibration UI has leftmost/rightmost note dropdowns, so a real user supplies the absolute anchor directly. Octave *inference* was only ever necessary because those dropdowns were not yet wired to the backend (§1). This is a test-harness limitation, not a product one. **As of item 16 this is resolved in testing too** — the dropdowns now reach the backend, so nothing needs to infer an octave any more.

16. **Calibration wiring — the UI's data finally reaches the backend, and the frame-edge clamping bug.** Two changes, both prerequisites for any future CV integration:
    - **Fixed: corner handles were clamped to `[0,1]`, i.e. to the frame itself, which made a correct calibration literally unexpressible for close-up shots.** Real keybeds routinely run off the edge: `test.wav.mp4`'s front-left corner is at x=-17.6px on a 640px frame, and `chords-notes-mix.mp4`'s front-right at x=259.5px on a 240px frame — so *both* reference clips were uncapturable through the UI. Handles now move on a padded stage spanning `[-0.25, 1.25]` in frame-normalized units (`HANDLE_MARGIN`/`HANDLE_SPAN`), with the preview letterboxed inside it. Bounded rather than unbounded so a stray drag can't produce a wild homography. The margin is 0.25 rather than something larger on purpose: it gives ~9× the headroom actually needed while keeping the preview at 2/3 of the card width, and since drag precision *is* calibration accuracy (see §5's precision-floor entry), shrinking the image to buy unused margin would trade away the thing that matters. **Verified by real browser drags, not just by changing the constant** — front-left was dragged to x=-16.2 and front-right to x=641.8 on a 640px frame, both accepted and round-tripped correctly.
    - **Wired: calibration now flows UI → `/api/transcribe` → response.** Payload shape, coordinate space, validation, and the verification-only guarantee are all documented in §1 and §2. End-to-end proof was a browser-driven run (drag all four corners, pick B2/F6, submit) whose returned calibration was rendered back as an overlay on the real frame and visually confirmed to land on the real keys — the check that matters, since *"a plausible-looking JSON response with a misaligned overlay is exactly the failure mode that produced the tritone bug."*
    - Regression evidence: the committed pre-change `main.py` and the new one were loaded side by side in one process and `analyze_audio` deep-compared on both clips — **identical**. The generated LilyPond source is byte-identical between calibrated and uncalibrated runs, and no `calibration` key appears in the response when none is sent.
...
- **Octave inference is no longer necessary anywhere — the user supplies the anchor directly.** The calibration UI's leftmost/rightmost note dropdowns now reach `/api/transcribe` (§3 item 16), so both production *and* testing take the absolute note range as given rather than deriving it. This removes the entire class of failure that produced the ~80mm tritone offset (§3 item 15) at its root, rather than mitigating it. Corollary for future sessions: **if you find yourself inferring an octave from finger positions again, stop — send a note range instead.** The rule below still applies to any situation where an anchor genuinely must be derived.
...
- **MediaPipe 0.10.35: `mp.solutions.hands` is GONE, not deprecated.** Only `mp.tasks.vision.HandLandmarker` exists. Requires a separately-downloaded `.task` model at `backend/models/hand_landmarker.task` (gitignored, ~7.8MB, official Google storage URL — see `main.py:573-576` comment).
...
- **`note_type` labels are dead weight in the API response** — `calculate_note_durations` produces `"quarter"`/`"half"`/etc. and `analyze_audio` attaches them to each event (`main.py:1175`), but **nothing reads them**: the notation path (`spell_rhythm` / `_build_staff_input`) works purely from `duration_beats` in sixteenth-note integer units, and `frontend/app/page.tsx` neither displays the field nor declares it on its `EventData` type. Verified by grep across backend and frontend. Left in place pending a decision — removing them would change the public API response shape.
- **`generate_placeholder_pdf` (`main.py:38`) confirmed still present and still fully dead code** — re-verified in the consolidation pass: exactly one occurrence in the entire file, its own `def`, zero call sites. Safe to delete if asked; left alone because not explicitly requested.
- **⚠️ Manually-dragged calibration has a real precision floor: ~1.2mm systematic bias, 1.34mm median error, 3.26mm max.** Measured against the frame's own black keys on `test.wav.mp4` (17 in-range black keys × 50 scanlines, comparing each detected black-key centre's projected `u` against that key's canonical `u_center`). **Analytically-fitted corners score 0.40mm median / 2.37mm max / −0.12mm bias on the identical metric**, so roughly two-thirds of the manual error is the hand-drag itself, not the homography. Context for judging it: a white key is 23.6mm and a black key 12.7mm, so note *mapping* is correct with wide headroom, and this is ~25× smaller than the 80mm tritone offset that invalidated an earlier session (§3 item 15). But it is a genuine floor for anything **distance-based**: a proximity-veto threshold sitting in a ~19mm gap (§3 item 14) would be spending a meaningful fraction of that gap on calibration error alone. If distance-based filtering is ever adopted, either fit corners analytically or budget for this.
  *Caveat on the metric itself:* a first attempt reported 42mm max even for the known-exact analytic calibration. The cause was that the frame's outermost detected black keys (A#2, F#6) fall **outside** the B2–F6 calibrated range and were being matched to the wrong key by a nearest-neighbour search. The figures above come from the corrected version, which identifies each detected key by its index in the black-key pattern and skips out-of-range keys. Any future re-measurement should keep that guard.
- **CV measurement layer still unwired** — `detect_key_press_at_timestamp`, `detect_hand_canonical_positions`, and `lateral_distance_to_nearest_landmark_mm` have zero call sites (verified). `build_keyboard_calibration` is now wired but verification-only. Not a bug, just unfinished integration — see §1/§2.
...
Status note (calibration-wiring session): items 1–3 below are still **not started**, but two things around them have changed and the framing below is partly superseded. (a) Calibration now flows from the UI to the backend (§3 item 16), so anything here can take a user-supplied note range instead of inferring one. (b) The "bottleneck is z-signal discrimination" framing predates §3 item 14, which **abandoned the z signal entirely** in favour of 2D lateral proximity — so item 1 (VIDEO tracking mode) is only worth doing if the z approach is revived, and item 2's lateral scoring already has measurement code (`detect_hand_canonical_positions`, `lateral_distance_to_nearest_landmark_mm`), just no pipeline call site. Re-read §3 items 14–16 before picking any of these up.

In order:

1. **Switch MediaPipe from per-frame IMAGE mode to VIDEO tracking mode.** Current `detect_key_press_at_timestamp` runs independent per-frame `IMAGE`-mode detection and stitches fingertip identity across frames with a nearest-position heuristic (`main.py:703-707`) — a likely noise source, since there's no persistent hand/finger identity the way MediaPipe's own `VIDEO`/`LIVE_STREAM` tracker would provide.
