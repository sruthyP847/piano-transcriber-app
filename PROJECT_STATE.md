...
| 6 | Merge same-pitch fragments (gap ≤0.05s, span cap 2.0s) | `deduplicate_notes` |
...
|---|---|---|
| `DEDUP_GAP_SECONDS` / `DEDUP_MAX_SPAN_SECONDS` | 0.05 / 2.0 | dedup |
...
- **`generate_placeholder_pdf` (`main.py:37`) confirmed still present and still fully dead code** — zero call sites anywhere in the file (verified via grep this session). Safe to delete if asked; left alone because not explicitly requested.
...
Octave registers not precisely specified by the user beyond "mostly around octave 4" — judge pitch-class correctness first, register plausibility second.
